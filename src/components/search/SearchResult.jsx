...
import { PlayCircle, Play, Eye } from "lucide-react";
...
export default function SearchComponent() {
  const { fetchSongs, songs, fetchAlbums, albums,Topresult,setTopresult } = useFetch();
              ...
              <div className="w-[90vw] sm:w-full lg:w-1/3">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute bottom-4 right-4 lg:opacity-0 lg:translate-y-8 lg:scale-75 lg:group-hover:opacity-100 lg:group-hover:translate-y-0 lg:group-hover:scale-100 transition-all duration-300 ease-out">
                    <button onClick={()=>{setMusicId(Topresult.id)}} className="bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-lg transition-colors duration-200">
                      <Play size={24} />
                    </button>
                  </div>
                </div>
                ...
                <ScrollArea className="h-[40vh] sm:h-[50vh]">
                  <ul className="space-y-2 pr-4">
                    {songs.map((song, index) => (
                      <li
                        key={index}
                        className="flex items-center justify-between p-2 sm:p-3 rounded-lg transition-all hover:bg-secondary hover:scale-105 duration-300 "
